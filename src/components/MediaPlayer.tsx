...
export function MediaPlayer({ fileUrl, fileType, thumbnail }: MediaPlayerProps) {
  ...
  const debouncedHandleMouseEnter = useCallback(debounce(handleMouseEnter, 100), []);
  const debouncedHandleMouseMove = useCallback(debounce(handleMouseMove, 100), []);
  const memoizedRenderControls = useCallback(renderControls, [currentTime, duration, isMuted, isPlaying, playbackRate, volume]);
    ...
    if (isMinimized) {
      return (
        <FloatingBox onClose={toggleMinimize} onExpand={toggleMinimize}>
          ...
          {controlsVisible && memoizedRenderControls()}
        </FloatingBox>
      );
    } else {
      if (mediaType === 'video/mp4' || mediaType === 'video/webm') {
  ...
  }, [queue, isMinimized, fileUrl, fileType, thumbnail, isPlaying, controlsVisible, memoizedRenderControls]);
  ...
  const renderControls = () => (
    <div 
      ...
      onMouseMove={debouncedHandleMouseMove}
    >
      {/* Your existing renderControls content */}
    </div>
  );

  // Remaining code for toggleFullScreen, exitFullScreen, toggleMinimize, formatTime, FloatingBox component, etc.
